...
def get_city_route_no_db(
    ...
    if city_short_route is None:
        raise HTTPException(status_code=404, detail="No road")
...
def get_city_route_with_db(
    ...
    if x_api_key != "123321":
        raise HTTPException(status_code=401, detail="Wrong api key")
    city_short_route = min_city_distance_db(
        city_start, city_finish, city_distance_service
    )
    if city_short_route is None:
        raise HTTPException(status_code=404, detail="No road")
    return city_short_route
...
def add_city_to_db(
    ...
    if x_api_key != "123321":
        raise HTTPException(status_code=401, detail="Wrong api key")
    city_distance_service.add_city_distance(
        {
            "city_from": city_item.city_from,
            "city_to": city_item.city_to,
            "distance": city_item.distance,
        }
    )
