...
def min_city_distance_db(
    city_start: int, city_finish: int, city_distance_service: ServiceCityDistance
) -> Union[dict, None]:
    ...
    route_lst: List = [[math.inf, False, []] for _ in set_cities]
    ...
    for _ in set_cities:
        ...
        for city_to, dst in zip(
            (row.city_to for row in rows), (row.distance for row in rows)
        ):
    ...
    if len(route_lst[city_finish][2]) < 2:
        ...
        result = to_json_db(route_lst[city_finish][2], route_lst[city_finish][0])
